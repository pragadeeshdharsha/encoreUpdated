...
func (r *rich) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	if function == "changeColour" {
		return changeColour(stub, args)
	} else if function == "new1" {
		return new1(stub, args)
	}
	return shim.Error("No function exists")
}

func new1(stub shim.ChaincodeStubInterface, args []string) pb.Response {
...
func changeColour(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	for i := 0; colourIditerator.HasNext(); i++ {
		...
		fmt.Println("x.namespace", x.Namespace)
